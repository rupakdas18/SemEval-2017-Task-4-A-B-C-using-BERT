...
test_data_df = pd.read_csv("data/twitter-2016test-CE.txt", delimiter='\t', header=None, names=['id','topic', 'label', 'tweet'])
#test_data_df = pd.read_csv("data/twitter-2016devtest-CE.txt", delimiter='\t', header=None, names=['id','topic', 'label', 'tweet'])
...
# Train the Model
model = MultinomialNB().fit(X_train_tfidf.reshape(train_data,16054*2), train_labels)
...
predicted = model.predict(X_test_tfidf.reshape(test_data,16054*2))


# Find the Accuracy, precision, recall and F1 score
test_acc =accuracy_score(test_labels, predicted)
test_f1 = metrics.f1_score(test_labels, predicted, labels=[0, 1, 2,3,4], average='macro')
test_precision = precision_score(test_labels, predicted,labels=[0, 1, 2,3,4], average='macro')
test_recall = recall_score(test_labels, predicted,labels=[0, 1, 2,3,4], average='macro')
